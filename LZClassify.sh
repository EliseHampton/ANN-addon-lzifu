...
## ======================== Welcome to LZClassify ========================= ##
# LZClassify is a wrapper bash script to run classifier
# LZClassify runs several programs. one after the other, to either train or to
# predict the answers using a pre-trained set of parameters. In either
# case it can be run from here.
# LZClassify calls both python and octave scripts (You will need both installed
# before continuing)

# LZClassify uses the output from fitting 1, 2, and 3 component fits with
# lzifu (Ho et al 2016)
...
# LZClassify written for the S7 collaboration (PI: A/Prof Michael Dopita) and
...
# 21st Jul 2016 - Renamed to better identify after publication
## =================================================================== ##
clear
...
# classifier takes input in a text file CreateInput will take a LZIFU cube and turn it into the
# input needed for classifier
#call create input
...
foo="Calling classifier.m with train($train) and test($testt)."
echo ${foo}
echo ' '
START2=$(date +%s.%N) #take the time
/Applications/Octave.app/Contents/Resources/bin/octave --silent --eval "classifier/classifier" ${train} ${testt}
